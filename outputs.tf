...
output "cdn_frontdoor_rules_actions" {
  ...
  value       = { for k, v in azurerm_cdn_frontdoor_rule.cdn_frontdoor_rules : k => v.actions if v.actions != null && length(v.actions) > 0 }
}
output "cdn_frontdoor_rules_behaviour_on_match" {
  ...
  value       = { for k, v in azurerm_cdn_frontdoor_rule.cdn_frontdoor_rules : k => v.behaviour_on_match if v.behaviour_on_match != null && length(v.behaviour_on_match) > 0 }
}
...
output "cdn_frontdoor_rules_conditions" {
  ...
  value       = { for k, v in azurerm_cdn_frontdoor_rule.cdn_frontdoor_rules : k => v.conditions if v.conditions != null && length(v.conditions) > 0 }
}
